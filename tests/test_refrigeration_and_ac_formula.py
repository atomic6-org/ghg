...
from atomic6ghg.formulas.refrigeration_and_ac import null_replacer
...
def canonical_data():
    ...
    with open(TEST_FILENAME, 'r', encoding='utf-8') as canonical_instance:
        refrigeration_and_ac_document = json.load(canonical_instance)
        return refrigeration_and_ac_document


@pytest.fixture
def calculated_data(canonical_data):
    refrigeration_and_ac = RefrigerationAndAc(wks_data=canonical_data)
    return refrigeration_and_ac
...
def test_make_screening_method(canonical_data, calculated_data):
    ...
    for i, canonical_row in enumerate(canonical_data[key]):
        ...
        calculated_gas = calculated_row['gas']
        calculated_new_units_charge = canonical_row['newUnitsCharge']
        calculated_capacity_operating_units = canonical_row['capacityOperatingUnits']
        calculated_capacity_disposed_units = canonical_row['capacityDisposedUnits']
...
def test_output_schema_compliance(calculated_data, canonical_data, refrigeration_and_ac_schema):
    output = calculated_data.to_dict()
    output['version'] = canonical_data['version']
    refrigeration_and_ac_schema.validate(output)
